  ...
  return (
    <div className="productItem shadow-lg rounded-md overflow-hidden border-1 border-[rgba(0,0,0,0.1)]">
      ...
      bg-[rgba(0,0,0,0.7)] z-[60] p-3 gap-2">
        ...
        {/* Vendor Badge with Verified */}
        {(() => {
          const vendorName = item?.vendor?.storeName || item?.vendorShopName;
          if (vendorName) {
            return (
              <div className="absolute bottom-2 left-2 z-40 flex items-center gap-1 bg-white/95 backdrop-blur-sm rounded-lg px-2 py-1 shadow-md">
                <span className="text-[8px] lg:text-[9px] font-[600] text-gray-700">
                  {vendorName}
                </span>
                <span className="text-[8px] lg:text-[9px] font-[700] text-green-600 flex items-center gap-0.5">
        ...
        {(() => {
          ...
          // Adjust position if vendor badge exists
          const vendorName = item?.vendor?.storeName || item?.vendorShopName;
          const topPosition = vendorName ? 'top-[50px]' : 'top-[10px]';
          
          if (isOnSale && discountPercent > 0) {
            return (
              <span className={`sale-badge flex items-center absolute ${topPosition} left-[10px] z-50 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-lg px-2 py-1 text-[10px] lg:text-[11px] font-[700] shadow-lg`}>
                🔥 {discountPercent}% OFF
          </span>
            );
          }
